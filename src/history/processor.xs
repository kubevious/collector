...
export class HistoryProcessor
{
    ...
    lockForCleanup(cb: (handler: HandlerCallback) => void)
    {
        ...
        const handler = {
            finish: () => {
                ...
                const waiters = this._dbCleanupWaiters;
    ...
    accept(state: RegistryBundleState)
    {
        return this._acquireProcessLock()
            ...
            .finally(() => {
                ...
                const waiters = this._processFinishListeners
    ...
    private _processSnapshot(processableSnapshot: ProcessableSnapshot)
    {
        ...
        return Promise.resolve()
            ...
            .then(() => {
                ...
                const itemsDelta = this._produceDelta(snapshot, this._latestSnapshot!);
                const deltaSummary = this._constructDeltaSummary(snapshot, itemsDelta);
                ...
                return Promise.resolve()
                    ...
                    .then(result => {
                        ...
                        const list = _.values(tablesPartitionsData.byTable);
    ...
    {
        const newHashes = configHashes.filter(x => !this._configHashesDict[<string>x.config_hash]);
    ...
    {
        const tablesPartitionsData : TablesPartitionsInfo = {
            maxPartition: 0,
            byTable: {}
        };
        return Promise.serial(HISTORY_TABLES, tableName => {
            return this._database.queryPartitions(tableName)
                .then(partitions => {
                    ...
                    {
                        const partitionId = partitionInfo.value - 1;
    ...
    private _persistDiff(snapshot: DBSnapshot<SnapItemWithConfig>, partition: number, itemsDelta: DBRawDiffItem[], deltaSummary: DeltaSummary)
    {
        ...
        return Promise.resolve()
            ...
            .then(dbDiff => {
                ...
                const diffSnapshot = new DBSnapshot<DBRawDiffItem>(null);
    ...
    private _constructDeltaSummary(snapshot: DBSnapshot<SnapItemWithConfig>, itemsDelta: DBRawDiffItem[])
    {
        ...
        const currentSnapshotAlerts = this._constructAlertsSummary(snapshot);
        ...
        for(const kind of _.keys(currentSnapshotAlerts))
        {
            const dict = currentSnapshotAlerts[kind];
        ...
        const deltaAlertsDict = _.cloneDeep(currentTotalAlerts);
        const deltaAlertsByKindDict = _.cloneDeep(currentByKindAlerts);
        if (this._latestSnapshotAlerts)
        {
            for(const kind of _.keys(this._latestSnapshotAlerts))
            {
                if (!deltaAlertsByKindDict[kind]) {
                    deltaAlertsByKindDict[kind] = this._newAlertsDict();
                }
                const dict = this._latestSnapshotAlerts[kind];
                for(const itemAlerts of _.values(dict))
                {
    ...
    private _constructAlertsSummary(snapshot: DBSnapshot<SnapItemWithConfig>) : AlertsSummary
    {
        ...
        for(const item of snapshot.getItems().filter(x => x.config_kind == 'node'))
        {
            if (item.config.selfAlertCount)
            {
                for(const severity of _.keys(item.config.selfAlertCount))
                {
                    const count = item.config.selfAlertCount[severity];
                    if (count > 0)
                    {
    ...
    private _produceDelta(targetSnapshot: DBSnapshot<SnapItemWithConfig>, currentSnapshot: DBSnapshot<DBRawSnapItem>) : DBRawDiffItem[]
    {
        ...
        const itemsDelta : DBRawDiffItem[] = [];

        for(const key of targetSnapshot.keys)
        {
            const targetItem = targetSnapshot.findById(key)!;
            const currentItem = currentSnapshot.findById(key);
            let shouldAdd = true;
    ...
    {
        const configHash = HashUtils.calculateObjectHash(config);
        const item = <SnapItemWithConfig>baseData;
